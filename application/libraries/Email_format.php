...
class Email_format {
        ...
        <div style=\"font-family: Arial, sans-serif; background: #f7f7f7; padding: 32px;\">
            <div style=\"max-width: 600px; margin: auto; background: #fff; border-radius: 8px; box-shadow: 0 2px 8px #0001; padding: 0 0 32px 0;\">
                ...
                <div style=\"padding: 32px;\">
                    ...
                    <p>The Laboratory Code <strong>{$transHeader['ext_lab_code']}</strong> has been marked as <strong>{$notification}</strong>.</p>
    ...
    public function generateEmailNotificationByNutritionist($transHeader, $trans_detail_id, $recipient)
    {
        ...
        $recipientName = trim("{$recipient['nutritionist_name']}") ?: 'User';

        $sample = $this->CI->db->select('s.sample_name,td.release_ref_number')
            ->from('trans_details td')
            ->join('samples s', 's.id = td.sample_id', 'left')
            ->where('td.trans_detail_id', $trans_detail_id)
            ->get()
        ...
        $subject = "Job Order {$transHeader['job_order_no']} - COA Releasing Notification";
        ...
        <div style=\"font-family: Arial, sans-serif; background: #f7f7f7; padding: 32px;\">
            <div style=\"max-width: 600px; margin: auto; background: #fff; border-radius: 8px; box-shadow: 0 2px 8px #0001; padding: 0 0 32px 0;\">
                ...
                <div style=\"padding: 32px;\">
                    <h2 style=\"color: #2d6cdf; margin-top: 0;\">Job Order {$transHeader['job_order_no']} - COA Releasing</h2>
                    <p>Hi <b>{$recipientName}</b>,</p>
                    <p>We are pleased to inform you that the Certificate of Analysis (COA) for Laboratory Code <strong>{$transHeader['ext_lab_code']}</strong> with a Reference Number of <strong>{$sample['release_ref_number']}</strong> has been Released.</p>
                    <p style=\"color: #888; font-size: 13px;\">&copy; " . date('Y') . " Bounty Laboratory Info Management System. All rights reserved.</p>
                </div>
            </div>
        </div>
