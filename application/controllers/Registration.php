...
class Registration extends CI_Controller {
	...
	*/
	...
	public function index(){
		...
		$data['js_file'] = 'assets/js/registration.js?v=2.0';
	...
	public function get_lab_tests($laboratory_id = null)
	{
		...
		$this->db->select('lt.test_id, t.test_name, lt.sample_type_id, st.sample_type_name');
		...
		foreach ($data as $row) {
			$tests[$row->test_id] = $row->test_name;
...
public function submit_registration()
{
    ...
    if (!empty($_FILES['attachFile']['name'])) {
        ...
        if (!$this->upload->do_upload('attachFile')) {
            ...
            $headerData['attach_file'] = $this->upload->data('file_name');
        }
    }
	...
	$mmyy = date('my');
	$this->db->like('job_order_no', "-$mmyy-", 'both');
	$totalThisMonth = $this->db->count_all_results('trans_headers');
	$increment = str_pad($totalThisMonth + 1, 4, '0', STR_PAD_LEFT);
    ...
    foreach ($sampleNames as $index => $sampleID) {
        ...
        $detailData = [
            ...
            'plate_number_id'    => $plateNumbers[$index] ?: null,
            'batch_number_id'    => $batchNumbers[$index] ?: null,
